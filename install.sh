...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENCE NEXA BOT            [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "NEXA GANTENG" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu Geh Ke @Nexa_Xz${NC}"
    echo -e "${YELLOW}TELEGRAM : @Nexa_Xz${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©ɴᴇxᴀ ᴅᴇᴠ${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${BLUE}'ggg      ,gg'''  .    ggg       Auto Installer @NEXABOT Private   ${NC}"
  echo -e "${BLUE}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${BLUE}ggg:     gg.     -   ,ggg       • Telegram : t.me/NEXA_BOT     ${NC}"
  echo -e "${BLUE} ggg:     ggg._    _,ggg        • Creadit  : @NEXABOT ${NC}"
  echo -e "${BLUE} ggg.    '.'''ggggggp           • Support by @NEXABOT  ${NC}"
